...
<main class="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900/50 p-5 card-hover">
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    <button class="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors" title="Redaguoti">
                        ✏️
                    </button>
                    <button class="p-2 rounded-lg text-slate-400 hover:text-<?= $search['active'] ? 'orange' : 'emerald' ?>-400 hover:bg-slate-800 transition-colors"
                            title="<?= $search['active'] ? 'Sustabdyti' : 'Aktyvuoti' ?>">
                        <?= $search['active'] ? '⏸️' : '▶️' ?>
                    </button>
                    <button class="p-2 rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-800 transition-colors" title="Ištrinti">
                        🗑️
                    </button>
                </div>
            </div>
    ...
    <!-- New Search Modal (hidden by default) -->
    <div id="newSearchModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div class="w-full max-w-lg mx-4 rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            ...
            <form class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-400 mb-1">Kategorija</label>
                    <select class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-<?= $meta['color'] ?>-500">
                        ...
                        <option>Buitinė technika</option>
                        <option>Baldai</option>
                        <option>Kita</option>
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-400 mb-1">Raktažodis</label>
                    <input type="text" placeholder="pvz. M paketas, 4K OLED..."
                           class="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-<?= $meta['color'] ?>-500">
                </div>
                <div>
                ...
                <div class="flex gap-3 pt-2">
                    <button type="button"
                            onclick="document.getElementById('newSearchModal').classList.add('hidden')"
