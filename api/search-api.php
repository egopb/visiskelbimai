<?php
/**
 * Search CRUD API (Vercel version) — uses /tmp/ for persistent storage fallback
 */
...
// Use /tmp for Vercel (writable), fall back to data/ for local
...
function buildSearchUrl(string $platform, array $params): string {
    switch ($platform) {
        case 'skelbiu':
            ...
            $categoryPaths = [
                'Automobiliai' => 'automobiliai', 'Kompiuteriai' => 'kompiuteriai',
                'Telefonai' => 'telefonai', 'Buitinė technika' => 'buitine-technika', 'Baldai' => 'baldai',
            ];
        ...
        case 'aruodas':
            $typeMap = [
                'Butai pardavimui' => 'butai', 'Butai nuomai' => 'butu-nuoma',
                'Namai pardavimui' => 'namai', 'Sklypai' => 'sklypai', 'Patalpos' => 'patalpos',
            ];
            $type = $typeMap[$params['Tipas'] ?? 'Butai pardavimui'] ?? 'butai';
            $cityMap = [
                'Vilnius' => 'vilniuje', 'Kaunas' => 'kaune', 'Klaipėda' => 'klaipedoje',
                'Šiauliai' => 'siauliuose', 'Panevėžys' => 'panevezyje',
            ];
...
function scanPlatform(string $url): int {
    ...
    foreach ($patterns as $p) {
        if (preg_match($p, $html, $m)) return (int)preg_replace('/\s/', '', $m[1]);
    }
    return 0;
}
...
switch ($action) {
    case 'create':
        ...
        if ($name === '') { echo json_encode(['error' => 'Pavadinimas privalomas']); exit; }
        ...
        if (!empty($_POST['category'])) $newSearch['category'] = $_POST['category'];
        $searches[] = $newSearch;
    ...
    case 'toggle':
        ...
        foreach ($searches as &$s) {
            if ($s['id'] === $id) { $s['active'] = !$s['active']; $found = true; break; }
        }
        unset($s);
